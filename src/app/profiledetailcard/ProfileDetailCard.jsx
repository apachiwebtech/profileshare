...
const ProfileDetailCard = ({ userId }) => {
    ...
    async function getDatapost() {
        const data = {
            slug: userid || 'bhoomika-ravikumar',
        };
        axios
            .post(`${BASE_URL}/shared_posts`, data)
            .then((res) => {
                setgetpost(res.data);
            })
            .catch((err) => {
    ...
    return (
        <div>
            {post?.map((item, index) => {
                ...
                return (
                    <div className="posty" key={item.post_id ?? index}>
                                                ...
                                                <li key={filteredLike.post_id}>
                                                    {filteredLike.like_count !== null ? (
                                                        <>
                                                            ...
                                                            <img onClick={() => onhandlelikeuser(item.post_id)} type='button' data-bs-toggle='modal' data-bs-target='#exampleModal2' src={likeimg} alt="" />
                                                            <span onClick={() => onhandlelikeuser(item.post_id)} type='button' data-bs-toggle='modal' data-bs-target='#exampleModal2'> {filteredLike.like_count}</span>
                                                        </>
                                                    ) : (
                                                        <>
                                                            <Link href="#" onClick={() => handleLike(item.post_id)}><i className={likedata.some((ele) => ele.post_id === item.post_id) ? 'fas fa-heart text-danger' : 'fas fa-heart '} id='like1' ></i> Like</Link>
                                            ...
                                            return (
                                                <li key={filteredLike.post_id} className='px-2'>
                                                    {filteredLike.comment_count !== null ? (
                                                        <Link href="#" className="com" onClick={() => { handlecomment(index); dispatch(getCount(item.post_id)); }}><i className="fas fa-comment-alt"></i> Comment   {filteredLike.comment_count} </Link>
                                                    ) : (
                                                        <Link href="#" className="com" onClick={() => { handlecomment(index); dispatch(getCount(item.post_id)); }}><i className="fas fa-comment-alt"></i> Comment  0 </Link>
                                                    )}
                                                </li>
                                            );
                                        })}
                        ...
                        {commentshow[index] && <div className="comment-section">
                                            ...
                                            <li>
                                                <div className="comment-list">
                                                    ...
                                                    <div className="comment">
                                                        ...
                                                        <p>{item.comment} </p>
                                                        {deleteuserid == item.user_id && <Link onClick={() => {
                                                        ...
                                                        }} title="" className="active"><i className="fa-solid fa-trash-can"></i>Delete</Link>}
                            ...
                            <div className="post-comment">
                                ...
                                <div className="comment-sec d-flex align-items-center">
                                    ...
                                    <i className="lar la-paper-plane px-2" style={{ cursor: "pointer", fontSize: "36px" }} onClick={() => {
